...
st.caption("Lecture simple ou multiple de plaques · Recherche constructeur · Statut obsolescence · Remplacement proposé")
...
if uploaded_files:
    ...
    st.info(
        "Tu peux lire toutes les plaques en une seule fois. "
        "Les boutons individuels restent disponibles pour relancer une seule image si besoin."
    )

    col_global_1, col_global_2 = st.columns([1, 2])

    with col_global_1:
        lire_tout = st.button("Lire toutes les plaques importées avec IA", type="primary")

    with col_global_2:
        st.caption(
            "Attention : chaque image analysée consomme un appel API. "
            "Pour économiser le crédit, importe seulement les photos utiles."
        )
    ...
    if lire_tout:
        ...
        st.success("Lecture globale terminée. Vérifie les informations extraites.")
        st.rerun()
