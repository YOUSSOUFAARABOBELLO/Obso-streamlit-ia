...
st.caption("Lecture de plaque · Recherche constructeur · Statut obsolescence · Remplacement proposé")

st.markdown(
    """
Cette application permet de charger une photo de plaque signalétique, d’identifier l’équipement,
puis de rechercher les informations de cycle de vie auprès du constructeur ou de sources fiables.
"""
)
...
with st.sidebar:
    ...
    st.write("Modèle lecture plaque :", os.getenv("OPENAI_MODEL", "gpt-4o-mini"))
    st.write("Modèle recherche web :", os.getenv("OPENAI_SEARCH_MODEL", "gpt-4o-mini"))
    st.warning("Chaque lecture IA et chaque recherche constructeur consomme du crédit API.")
    st.markdown(
        """
**Cahier des charges :**
1. Charger photo  
2. Lire plaque  
3. Rechercher constructeur  
4. Statut cycle de vie  
5. Remplacement proposé  
6. Sources  
7. Export  
"""
    )
...
if uploaded_files:
    for idx, uploaded_file in enumerate(uploaded_files):
        ...
        with col_infos:
            ...
            st.info(
                "Lance cette recherche seulement après avoir vérifié le fabricant et la référence. "
                "L’objectif est de trouver ce que dit le constructeur ou une source fiable."
            )
